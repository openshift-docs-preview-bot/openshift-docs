...
[NOTE]
====
The ability to limit ephemeral storage is available only if an administrator enabled the xref:../../install_config/configuring_ephemeral.adoc#install-config-configuring-ephemeral-storage[ephemeral storage] technology preview. This feature is disabled by default.
====

A deployment is completed by a deployment pod that consumes resources (memory, CPU, and ephemeral storage) on a
compute node like other usual pods. By default, a deployment pod consumes unbounded node resources on the scheduled compute node. Usually, this unbounded resource consumption does not cause problems, because deployment pods consume a small amount of resources and run for a short period. However, if a project specifies default container limits, then pods including the deployment pod consume resources up to those limits.

You can also limit resource used by a deployment pod by specifying resource limits as part of the
...
The `ephemeral-storage` parameter is available only if an administrator enabled the xref:../../install_config/configuring_ephemeral.adoc#install-config-configuring-ephemeral-storage[ephemeral storage] technology preview.
====
