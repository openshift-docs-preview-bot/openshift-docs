...
{product-title} is going to update to HAProxy 2.2, which down-cases HTTP header names by default, for example, changing `Host: xyz.com` to `host: xyz.com`. Make sure to add the necessary configuration using `spec.httpHeaders.headerNameCaseAdjustments` before upgrading {product-title} when HAproxy 2.2 is available.
