...
By default, this is the namespace where the Operator runs. To watch all namespaces, you can leave the `namespace` option empty:
...
It is also possible to use the link:https://godoc.org/github.com/kubernetes-sigs/controller-runtime/pkg/cache#MultiNamespacedCacheBuilder[`MultiNamespacedCacheBuilder`] function to watch a specific set of namespaces:
...
<2> Creates a new `Cmd` to provide shared dependencies and start components.
