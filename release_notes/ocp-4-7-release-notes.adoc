...
You can now install a cluster on {rh-openstack-first} that uses bare metal machines. The cluster can have both control plane and compute machines running on bare metal, or just compute machines.

// Link when available.

[id="ocp-4-7-osp-validations"]
==== Validating {rh-openstack} requirements at installation

The {product-title} installer now performs a variety of checks before attempting to install a cluster on {rh-openstack}. The installer can identify certain conditions that cause installations to fail, such as overlapping subnets.

[id="ocp-4-7-osp-custom-subnets"]
==== Custom subnets for machines on {rh-openstack}

You can now configure machines in clusters that run on {rh-openstack} to use a network and subnet of your choice.

// Link when available. Custom subnets, again. 
...
// Link when available. Custom subnets section. 
...
You can now enable QEMU Guest Agent support by setting a property in the `install-config.yaml` file during installation.

[id="ocp-4-7-additional-pvs"]
==== Increasing the persistent volume limit for clusters on {rh-openstack}

You can now configure nodes to have more than 26 persistent Cinder volumes in clusters on {rh-openstack} by setting the `platform.openstack.clusterOSImage` property in the `install-config.yaml` file during installation.
