...
[NOTE]
====
If you are creating a local volume for cluster logging or {product-title} Monitoring, do not use a raw block volume (`volumeMode: block`) because Elasticsearch cannot use raw block volumes.
====
+
.Example: Block
[source,yaml]
