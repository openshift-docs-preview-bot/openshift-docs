...
* Currently, installation on IBM z15 and LinuxONE III fails. As a workaround, add the option `dfltcc=off` to `boot.parm` via the command line. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1903383[*BZ#1903382*])

* Currently, OCP nodes installed on FCP-based SCSI storage might crash. As a workaround, use DASD-based storage for the guest usage. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1899762[*BZ#1899762*])

* When installing {product-title} on IBM Z with KVM, the recommended memory of 8 GB for OCP nodes might not be sufficient. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1908875[*BZ#1908875*])
...
With this release, KVM on  {op-system-base} 8.3 or later is supported as a hypervisor for user-provisioned installation of {product-title} {product-version} on IBM Z and LinuxONE.  See xref:../installing/installing_ibm_z/installing-ibm-z-kvm.adoc#installing-ibm-z-kvm[Installing a cluster with {op-system-base} KVM on IBM Z and LinuxONE] for installation instructions.
...
* {product-title} Do (odo)
* {product-title} Pipelines TP
* {product-title} Service Mesh
...
* Worker nodes must run {op-system-first}.
* Persistent storage must be of the Filesystem mode using local volumes, Network File System (NFS), or Container Storage Interface (CSI)
