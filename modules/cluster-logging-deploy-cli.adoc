...
[NOTE]
====
If you are using a local volume for persistent storage, do not use a raw block volume (`volumeMode: block` in the `LocalVolume` object) because Elasticsearch cannot use raw block volumes.
====
...
<7> Specify the CPU and memory requests for Elasticsearch as needed. The defaults are `16G` for the memory request and `1` for the CPU request.
<8> Specify the memory requests for the Elasticsearch proxy as needed. The defaults are `256Mi` for the memory request and `100m` for the CPU request.
<9> Settings for configuring Kibana. Using the CR, you can scale Kibana for redundancy and configure the CPU and memory for your Kibana pods. For more information, see *Configuring Kibana*.
