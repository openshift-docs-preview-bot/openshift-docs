...
==== Enhanced disk provisioning for LUKS and RAID devices

{op-system} 4.7 includes several improvements to disk provisioning for bare metal deployments:
...
* {op-system} on s390x can be installed onto FBA DASD disks.

[NOTE]
====
LUKS configuration can no longer be performed in the `/etc/clevis.json` file. Provisioning fails if `/etc/clevis.json` is specified in the machine config.
====
...
==== Persistent storage using CSI volume snapshots is generally availabile
...
* `AffinityAndTaints`: evicts pods that violate basic pod and node affinity and node taints.
...
==== `AlertmanagerConfig` custom resource definition in Prometheus Operator not supported

Modifying Alertmanager configurations by using the `AlertmanagerConfig` custom resource definition (CRD) in Prometheus Operator is not supported.
...
[id="ocp-4-7-monitoring-namespace-by-pod-and-pod-total-Grafana-dashboards"]
==== Namespace By Pod and Pod Total Grafana dashboards

The Namespace By Pod and Pod Total networking Grafana dashboards are enabled. // What are these dashboards used for?

[id="ocp-4-7-monitoring-prometheus-config-reloader-memory-limits"]
==== `prometheus-config-reloader` memory limit removed for monitoring user defined projects

The memory limit was removed on the Prometheus Operator `prometheus-config-reloader` container in the `openshift-user-workload-monitoring` namespace for monitoring user defined projects. This update prevents OOM kill of the `prometheus-config-reloader` container because it used more memory than the defined limit.
...
==== Removed deprecated configuration for monitoring your own services

In {product-title} {product-version}, the previous Technology Preview configuration that enabled users to monitor their own services is now removed and not supported.
